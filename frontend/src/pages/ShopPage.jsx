/**
 * 
 * Shop page component for Momentum
 * 
 * Allows users to purchase items with coins earned from completed habits
 * Displays items like themes, powerups, and backgrounds
 * 
 * @dependencies
 *  - next/navigation
 *  - react hooks
...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    // Sample shop items - TODO: implement backend implementation for user data for just simulated data
    setTimeout(() => {
      // Set inventory items first
      setInventoryItems([
        {
          id: "theme-2",
  ...
  const isItemOwned = (itemId) => {
    return inventoryItems.some((item) => item.id === itemId)
  }

  const handlePurchase = (item) => {
    if (coins >= item.price && !isItemOwned(item.id)) {
              ...
              filteredItems.length > 0 ? (
                filteredItems.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      {/* Item Image - using a placeholder for now */}
                        ...
                        <div className="flex items-center mb-2">
                          ...
                          {item.usageLimit && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono bg-white/10 text-gray-300">
                              ...
                              {item.usageLimit} use{item.usageLimit > 1 ? "s" : ""}
                            </span>
                          )}
                        </div>
            ...
            >
              <div className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-center text-xs font-mono uppercase mb-4 ${getRarityColor(selectedItem.rarity)}">
